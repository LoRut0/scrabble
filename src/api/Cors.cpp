...
CorsHandler::CorsHandler(const components::ComponentConfig &config,
                         const components::ComponentContext &context)
    : HttpHandlerBase(config, context) {};

std::string
CorsHandler::HandleRequest(server::http::HttpRequest &request,
    ...
    resp.SetHeader(std::string{"Access-Control-Allow-Methods"},
                   "GET, POST, OPTIONS");
    return {};
}
