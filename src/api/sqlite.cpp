...
std::vector<int> WebsocketsHandler::sql_ongoing_get_all() const {
    std::vector<int> result =
        sqlite_client_
            ->Execute(storages::sqlite::OperationType::kReadOnly,
                      OngoingSelectAll.data())
            .AsVector<int>();
    return result;
}

std::string WebsocketsHandler::sql_ongoing_new() const {
    ...
    storages::sqlite::ExecutionResult result =
        transaction.Execute(OngoingInsertKeyValue.data()).AsExecutionResult();
    ...
    throw server::handlers::InternalServerError(
        server::handlers::ExternalBody{"SQL Insert error"});
}

int64_t WebsocketsHandler::sql_ongoing_delete(std::string_view key) const {
    const storages::sqlite::Query kDeleteValue{OngoingDeleteKeyValue.data()};

    storages::sqlite::ExecutionResult result =
        sqlite_client_
            ->Execute(storages::sqlite::OperationType::kReadWrite, kDeleteValue,
                      key)
            .AsExecutionResult();
